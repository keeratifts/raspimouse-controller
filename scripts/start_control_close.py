...
if __name__ == '__main__':
    try:
        ...
        velPub_0 = rospy.Publisher('raspi_1/cmd_vel', Twist, queue_size=10)
        ...
        while not rospy.is_shutdown():
            if not robot_in_pos:
                ...
                if len(raspi) == 2:
                    ...
                    for i in goal:
                            X_GOAL.append(goal[i]['x'])
                            Y_GOAL.append(goal[i]['y'])

                    raspi = closest_distance(X_GOAL, Y_GOAL, raspi)
                    robot_in_pos = True
                    print('\r\nInitial position:')
                    print('Raspi_0: (x, y, theta) = (%.2f, %.2f, %.2f)' % (raspi[1]['x'], raspi[1]['y'], degrees(raspi[1]['yaw'])))
                    ...
                    print('Raspi_0 goal : (%.2f, %.2f)' % (raspi[1]['GOAL_X'], raspi[1]['GOAL_Y']))
                    print('Raspi_2 goal : (%.2f, %.2f)' % (raspi[2]['GOAL_X'], raspi[2]['GOAL_Y']))
                    print('')

                    X_GOAL = [raspi[1]['GOAL_X'], raspi[2]['GOAL_X']]
                    Y_GOAL = [raspi[1]['GOAL_Y'], raspi[2]['GOAL_Y']]
                ...
                AlvarMsg = rospy.wait_for_message('/ar_pose_marker', AlvarMarkers)
                raspi = get_robot_location(AlvarMsg, raspi)
                status_2 = robotFeedbackControl_without_beta(velPub_2, raspi[2]['x'], raspi[2]['y'], raspi[2]['yaw'], X_GOAL[1], Y_GOAL[1])
                #status_1 = robotFeedbackControl_without_beta(velPub_1, raspi[1]['x'], raspi[1]['y'], raspi[1]['yaw'], X_GOAL[1], Y_GOAL[1])
                status_0 = robotFeedbackControl_without_beta(velPub_0, raspi[1]['x'], raspi[1]['y'], raspi[1]['yaw'], X_GOAL[0], Y_GOAL[0])
                #if status_0 == 'Goal Position reached' and status_1 == 'Goal Position reached' and status_2 == 'Goal Position reached':
                if status_0 == 'Goal Position reached' and status_2 == 'Goal Position reached':
                    robotStop(velPub_0)
                    robotStop(velPub_2)
                    rospy.signal_shutdown('End of testing')
